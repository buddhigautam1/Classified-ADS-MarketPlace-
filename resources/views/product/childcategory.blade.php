        ...
        <div class="row">
            <aside class="col-lg-3 mb-4">
                <div class="card filter-card mb-4">
                    <div class="card-header">Related filters</div>
                    <div class="card-body">
                        @forelse ($filterByChildCategories as $filterchildcategory)
                            <a class="filter-link" href="{{ $filterchildcategory->childcategory->slug ?? '#' }}">
                                <span>{{ $filterchildcategory->childcategory->name ?? '' }}</span>
                                <i class="fas fa-chevron-right"></i>
                            </a>
                        @empty
                            <p class="text-muted mb-0">No related filters available.</p>
                        @endforelse
                    </div>
                </div>
